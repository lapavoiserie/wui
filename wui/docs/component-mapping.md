...
| `VStack` -> `VStack` | `VStack` -> `StackPanel` (Vertical) | Identical API. Different native control. A panel measures its children with an unbounded height, so a `ScrollViewer` inside a VStack has nothing to scroll and a vertical `Spacer` takes no room: put the ScrollView at the surface's root. Making it a Grid, as `HStack` is, drew the pupitre entirely black -- laid out, unpainted -- and was put back the same day. |
...
| `Spacer` -> `Spacer` | `Spacer` -> `Border` (a starred column) | Same concept. An empty Border, tagged so the `HStack` around it gives its column the leftover width. In a `VStack` it takes none: a StackPanel cannot distribute height. A node type of its own: it used to report `Border` and set a property, so a *received* `Spacer` was drawn as the text `?Spacer`. |
...
| `clip` (a canon modifier) | `UIElement.Clip`, a sized `RectangleGeometry` | WinUI has **no property** for it: a Grid and a StackPanel do not clip, and every container here is one. A geometry does not follow its element either — one set before layout stays 0x0 for good — so it is re-sized on `SizeChanged`. **Limit:** a `clip` that disappears from a node's chain is not removed. The push contract has no "reset the modifiers" call, so nothing runs to undo it; a tree that clips sometimes and not others keeps the cut. |
